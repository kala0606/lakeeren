...
// Image upload: when set, used as source instead of webcam
let uploadedImg = null;
let useUploadedImage = false;
let imgSnapshot = null; // resized copy for contour extraction
...
var params = {
  gridRes:     520,    // grid width (height = 3/4 of this)
  edgeThresh:  30,     // base Sobel threshold
  minLen:      6,      // min contour length in grid pixels
  maxPts:      40,     // max points per contour for rendering

  // Sin wave displacement
  waveAmp:     0,      // amplitude in pixels (0 = off, sharp lines)
  ...
  bgG:         250,
  bgB:         245,
  bgAlpha:     255,     // background alpha (low = trail, 255 = solid)
...
function buildPanel() {
  ...
  html += "<b>Source</b><br>";
  html += '<input type="file" id="fileInput" accept="image/*" style="font-size:11px;margin-bottom:4px"><br>';
  html += '<button type="button" id="clearImageBtn" style="margin-bottom:8px;font-size:11px;padding:4px 8px;cursor:pointer;border-radius:4px;border:none;background:#555;color:#fff">Use camera</button><br>';
  html += "<hr style='border-color:#555'><br>";
  ...
  if (fileEl) {
    fileEl.addEventListener("change", function() {
      ...
      loadImage(url, function(img) {
        ...
        uploadedImg = img;
        videoAspect = img.width / img.height;
        recalcGrid();
        useUploadedImage = true;
        imgSnapshot = createImage(GRID_W, GRID_H);
      }, function() {
        URL.revokeObjectURL(url);
...
function draw() {
  ...
  if (useUploadedImage && uploadedImg) {
    if (!imgSnapshot || imgSnapshot.width !== GRID_W || imgSnapshot.height !== GRID_H) {
      imgSnapshot = createImage(GRID_W, GRID_H);
    }
    imgSnapshot.copy(uploadedImg, 0, 0, uploadedImg.width, uploadedImg.height, 0, 0, GRID_W, GRID_H);
    imgSnapshot.loadPixels();
    if (!imgSnapshot.pixels || imgSnapshot.pixels.length < GRID_W * GRID_H * 4) return;
    contourPaths = extractContours(imgSnapshot);
    renderContours();
  } else if (videoReady) {
    var snap = video.get();
    ...
    text("Waiting for camera...  or upload an image above", width / 2, height / 2);
    return;
  }
...
// Fast 3×3 box blur on the global bri[] array (in-place).
// Reduces high-frequency pixel noise before Sobel, giving cleaner edges.
function blurBrightness(w, h, passes) {
  var tmp = new Float32Array(w * h);
  for (var pass = 0; pass < passes; pass++) {
    for (var j = 1; j < h - 1; j++) {
      for (var i = 1; i < w - 1; i++) {
        var k = j * w + i;
        tmp[k] = (bri[k-w-1] + bri[k-w] + bri[k-w+1] +
                  bri[k-1]   + bri[k]   + bri[k+1]   +
                  bri[k+w-1] + bri[k+w] + bri[k+w+1]) * 0.11111;
      }
    }
    for (var j = 1; j < h - 1; j++) {
      for (var i = 1; i < w - 1; i++) {
...
function extractContours(img) {
  ...
  // Spatial box blur: smooths out pixel noise so Sobel sees clean edges
  blurBrightness(w, h, 2);
  ...
  for (var j = 1; j < h - 1; j++) {
    for (var i = 1; i < w - 1; i++) {
      ...
      var mag = gx*gx + gy*gy;
      var darkness = 1 - bri[k] * 0.0392;
      var thresh = et + (1 - darkness) * 10;
  ...
  for (var j = 1; j < h - 1; j++) {
    for (var i = 1; i < w - 1; i++) {
      ...
      // Smooth the raw pixel-grid staircase at the source before storing.
      // This means getContourPoints() will always sample from smooth coords.
      if (path.length >= minL) result.push(chaikinSmooth(path, 3));
    }
  }
...
function getContourPoints(flat) {
  ...
  // Final polish pass (raw path already Chaikin-smoothed in extractContours)
  if (params.smooth && points.length >= 8) {
    points = chaikinSmooth(points, 1);
  }
  return points;
}

// Chaikin corner-cutting smoothing on flat [x0,y0,x1,y1,...] array.
// Each iteration ~doubles points and rounds corners toward smooth curves.
function chaikinSmooth(pts, iters) {
  var out = pts.slice();
  for (var it = 0; it < iters; it++) {
    var next = [out[0], out[1]];
    for (var i = 0; i < out.length - 2; i += 2) {
      next.push(
        0.75*out[i]   + 0.25*out[i+2],
        0.75*out[i+1] + 0.25*out[i+3],
        0.25*out[i]   + 0.75*out[i+2],
        0.25*out[i+1] + 0.75*out[i+3]
      );
    }
    next.push(out[out.length-2], out[out.length-1]);
    out = next;
  }
  return out;
}
...
// Build a smooth SVG path string from flat [x0,y0,x1,y1,...] using
...
function catmullRomSVGPath(pts) {
  ...
  for (var i = 1; i < n; i++) {
    var i0 = Math.max(0,    i - 2) * 2;
    var i1 = (i - 1)              * 2;
    var i2 = i                    * 2;
    var i3 = Math.min(n - 1, i + 1) * 2;
    var p1x = pts[i1], p1y = pts[i1+1];
    var p2x = pts[i2], p2y = pts[i2+1];
    var cp1x = p1x + (pts[i2] - pts[i0]) / 6;
    var cp1y = p1y + (pts[i2+1] - pts[i0+1]) / 6;
    var cp2x = p2x - (pts[i3] - pts[i1]) / 6;
    var cp2y = p2y - (pts[i3+1] - pts[i1+1]) / 6;
...
function buildSVG() {
  ...
  for (var ci = 0; ci < pointsToUse.length; ci++) {
    ...
    if (params.smooth && pts.length >= 8) {
      d = catmullRomSVGPath(pts);
    } else {
      d = "M " + pts[0].toFixed(1) + "," + pts[1].toFixed(1);
      for (var p = 2; p < pts.length; p += 2) {
        d += " L " + pts[p].toFixed(1) + "," + pts[p+1].toFixed(1);
      }
    }
    pathsStr += '  <path d="' + d + '" fill="none" stroke="' + col + '" stroke-width="' + params.strokeW.toFixed(1) + '" stroke-linecap="round" stroke-linejoin="round"/>\n';
  }
